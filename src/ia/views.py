...
def selectraw(request):
    query = """
            SELECT * FROM pais, estado WHERE ( pais.pais_id = estado.pais_id ) 
            """
...
class PaisViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        for estado in var1:
            print(estado.estado_id, estado.nombre, estado.codigo, estado.pais_id)
        # var3 = var2[0].nombre
        # print("var3")
        # print(var3)
...
def reporte(request):
    ...
    response['Content-Disposition'] = 'attachment: filename=F:\Reporte.pdf'
    buffer = BytesIO()
    ...
    c.drawString(30, 750, 'Jose Rico')
    c.setFont('Helvetica', 12)
    c.drawString(30, 735, 'Reporte')
    
    c.setFont('Helvetica', 12)
    c.drawString(480, 750, '25/05/2017')
...
# class VistaPDF(TemplateView):
#
#     def get_context_data(self, **kwargs):
#         context = super(VistaPDF, self).get_context_data(**kwargs)
#         context['estados'] = Estado.objects.all()
#         return context

class VistaPDF(PDFView):
...
@api_view(['GET'])
def convpdf(request):
    # template = loader.get_template('convpdf.html')
    # data = {'a': '1', 'b': '2'}
    data = Estado.objects.all()
    # texto = loader.render_to_string('convpdf.html', data)
    # html = texto
    # pdf = pdfkit.from_string(html, 'convpdf.pdf')

    template = loader.get_template("convpdf.html")
    # context = Context({"data": data})  # data is the context data that is sent to the html file to render the output.
    context = Context({"estados": data})  # data is the context data that is sent to the html file to render the output.
    # context['estados'] = Estado.objects.all()
    html = template.render(context)  # Renders the template with the context data.
    content = pdfkit.from_string(html, "convpdf.pdf")
    # return HttpResponse(html)
    # pdfkit.from_string(html, "convpdf.pdf")
    # pdf = open("convpdf.pdf")
    # response = HttpResponse(html)
    # contenido = pdf.read()
    # response = HttpResponse(contenido, content_type='application/pdf')
    # response = HttpResponse(html)
    response = HttpResponse(content, content_type='application/pdf')
    # response['Content-Disposition'] = 'attachment; filename="convpdf.pdf"'
    response['Content-Length'] = len(content)
    # pdf.close()
    # os.remove("convpdf.pdf")  # remove the locally created pdf file.
    return response
